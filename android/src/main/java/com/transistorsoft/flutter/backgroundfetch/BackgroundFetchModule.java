...
public class BackgroundFetchModule implements MethodCallHandler {
    ...
    void onAttachedToEngine(Context context, BinaryMessenger messenger) {
        if (!mIsAttachedToEngine.compareAndSet(false, true)) return;
        ...
        mEventChannel = new EventChannel(messenger, EVENT_CHANNEL_NAME);
        mEventChannel.setStreamHandler(mFetchCallback);
    }

    void onDetachedFromEngine(FlutterPlugin.FlutterPluginBinding binding) {
        mContext = null;
